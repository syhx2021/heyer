...
/*
*chat_with_llm 与llm沟通
输入：
prompt（关于protocol的语法的）
model（指定LLM的类型）
tries（尝试获取回答的最大次数）
temperature（一个浮点数，调整语言模型生成回答的随机性或创造性的参数）
输出：answer
*/
char *chat_with_llm(char *prompt, char *model, int tries, float temperature)
{
    ...
    do
    {
        ...
        if (curl)
        {
            ...
            if (res == CURLE_OK)
            {
                ...
                else
                {
                    ...
                    sleep(60); // Sleep for a small amount of time to ensure that the service can recover
                }
        ...
        {
            printf("请求失败，%d秒后重试...\n", retry_interval);
            sleep(retry_interval); // 使用当前重试间隔等待
            retry_interval *= 2;  // 将等待时间翻倍，准备下一次重试
        }
...
/*
*construct_prompt_for_templates(char *protocol_name, char **final_msg)
输入：协议的名字
输出：prompt_grammars
功能：构建prompt——引导LLM对协议的语法生成
*/
char *construct_prompt_for_templates(char *protocol_name, char **final_msg)
{
    ...
    char *msg = NULL;
    //msg包含了rstp和http的实例模板，询问对于我们要的协议(protocol_name)的所有client request templates（客户端请求模板）
    ...
    *final_msg = msg;
    ...
    //prompt的语法，两个角色，system，系统的内容是You are a helpful assistant；user,内容是msg的内容
...
输入：
protocol_name
first_question(之前向LLM提出的第一个问题)
first_answer(LLM对第一个问题的回答)
输出：prompt
功能：构建prompt——引导LLM根据第一个问题和回答及协议名称来生成其他请求模板
*/
...
输入：
answer(LLM的回复-protocol的语法结构)
klist_t(gram) * grammar_list(链表，用于收集提取的语法)
输出：/
功能:语法提取与储存
将一串包含多个JSON格式模板的字符串拆分并转换成JSON对象的链表
*/
void extract_message_grammars(char *answers, klist_t(gram) * grammar_list)
{
    ...
    while (ptr < answers + len) //遍历answer
    {
...
void get_protocol_message_types(char *state_prompt, khash_t(strSet) * states_set)
{
    ...
    for (int i = 0; i < CONFIDENT_TIMES; i++)
    {
        ...
        // char *state_answer = chat_with_llm(state_prompt, "turbo", MESSAGE_TYPE_RETRIES, 0.5);
...
*enrich_sequence
输入：
char *sequence：要被丰富的原始序列字符串。
khash_t(strSet) * missing_message_types：一个散列表，包含了需要被添加到序列中的缺失消息类型。
输出：
返回一个新字符串
功能：
构建prompt,让llm在正确位置里添加缺失的序列
*/
char *enrich_sequence(char *sequence, khash_t(strSet) * missing_message_types)
{
    ...
    // char *response = chat_with_llm(prompt, "turbo", ENRICHMENT_RETRIES, 0.5);
