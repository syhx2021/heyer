 ...
 * @Date: 2024-04-09 15:32:42
 * @LastEditors: 颖 吴 syhx2021@outlook.com
 * @LastEditTime: 2024-04-10 23:44:43
 * @FilePath: \ChatAFL-master\ChatAFL\afl-fuzz.c
...
void setup_llm_grammars()
{
  ...
  {
    klist_t(gram) *grammar_list = kl_init(gram); //初始化一个用于存储从LLM获取到的语法项的链表
...
void get_seeds_with_messsage_types(const char *in_dir, khash_t(strSet) * message_types_set)
{
  ...
  int nl_cnt = scandir(in_dir, &nl_files, NULL, alphasort); //读取指定目录中的文件列表，并按字母顺序排序,nl_cnt为目录中文件数量
  if (nl_cnt < 0)
  {
  ...
  for (int i = 0; i < nl_cnt; i++) //遍历目录中文件
  {
...
int main(int argc, char **argv)
{
  ...
  srandom(tv.tv_sec ^ tv.tv_usec ^ getpid()); //随机数生成
